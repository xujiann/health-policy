...
// 在已加载的政策语料上，实时算出某组词逐年命中数（标题+摘要任一命中）
function computeSeries(words) {
  const idx = {};
...
function addCustomKeyword(raw) {
  ...
  const { total } = computeSeries(words);
  state.customLines.push({ label, words, color, total });
  $("#kw-input").value = "";
  renderChips();
  refreshChart();
}
...
function renderChips() {
  $("#kw-chips").innerHTML = state.customLines.map((c) =>
    `<span class="kwchip" style="background:${c.color}">${esc(c.label)}（${c.total}）` +
    `<button data-kw="${esc(c.label)}" title="移除">×</button></span>`).join("");
...
function buildDatasets() {
  ...
  const customs = state.customLines.map((c) => ({
    label: "★" + c.label,
    data: computeSeries(c.words).series,
    borderColor: c.color,
  ...
  state.chart = new Chart($("#chart").getContext("2d"), {
    ...
    options: {
      ...
      plugins: {
        ...
        title: { display: true, text: "卫生健康主题政策数量逐年变化" },
      },
...
function renderThemeList(theme) {
  ...
  const hits = state.policies.filter((p) =>
    words.some((w) => (p.t + " " + p.s).includes(w)));
  hits.sort((a, b) => b.d.localeCompare(a.d));
  $("#theme-count").textContent = `「${theme}」命中 ${hits.length} 篇（关键词：${words.join("、")}），列出最近 50 篇`;
