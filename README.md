...
政策解读、答记者问、吹风会、新闻稿等不作为独立政策条目收录；能够可靠匹配到具体政策文件的，
仅作为“政策解读”链接显示在该文件卡片下方。无法匹配的解读和其他新闻类内容不进入网站清单、
筛选和趋势统计。
...
| `Daily policy data update` | 每天 06:20（北京时间） | GitHub Actions 运行 `update_static_data.py`，发现新政策即更新 `site/data` 并触发 Pages 发布 |
